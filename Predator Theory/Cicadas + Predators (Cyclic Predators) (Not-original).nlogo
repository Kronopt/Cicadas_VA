  ...
  ask cicadas [
    if (ticks mod lf-duration-ticks) = begin-month [
      if random 100 < cicadas-reproduction-rate [
        ;; if (count cicadas with [ lf-duration-ticks = ([lf-duration-ticks] of self )] ) <= max-cicadas-per-cycle [  ;; se o número de cicadas com um ciclo de vida ultrapassar o limite estipulado, não há reprodução
