...
globals [current-year]

cicadas-own [life-cycle adult]
...
to go
  ...
  if ( ticks mod ticks-a-year = 0 ) [ set current-year current-year + 1 ]  ;; increments year
...
to setup-cicadas
  ...
  create-cicadas n-cicadas [
    ...
    set color orange
    set hidden? true  ;; cicadas start underground
    set life-cycle starting-life-cycle
    set adult true
  ]
end
...
to reproduce-cicadas
  if hidden? = false and adult = true and (count cicadas with [life-cycle = [life-cycle] of self and not adult] < max-cicadas-per-cycle) [
    let c cicadas with [hidden? = false and adult = true]  ;; cicadas able to mate
    let mate one-of cicadas-on neighbors

    if mate != nobody and (membership neighbors c) [
      ...
      hatch cicadas-progeny [
        ...
        if [life-cycle] of mate != life-cycle [  ;; if parents have diferent life-cycles, offspring will suffer mutation
          ...
          ifelse random 100 < 50
             [set life-cycle (life-cycle + n)]
             [if life-cycle > n [set life-cycle (life-cycle - n)]]
        ]
      ]
    ]
  ]
...
to-report membership [my-neighbors c] ;; true if there is a neighbor belonging to c
  let result false
  ask cicadas-on my-neighbors [
...
to emergence
  ifelse current-year mod life-cycle = 0 ;; Cicadas emerge if the current year corresponds to their life cycle
    [set hidden? false]
    [set hidden? true]
end
...
to catch-preys
  let a-prey one-of (turtle-set preys-here cicadas-here with [hidden? = false])
  if a-prey != nobody [
    ask a-prey [ die ]
...
to move
  if not is-cicada? self [ set energy energy - 1 ]  ;; Cicadas don't have energy
  right random 50
  left random 50
  forward 1
end
...
PENS
"default" 1.0 1 -13840069 true "" "histogram ( [life-cycle] of cicadas )"
