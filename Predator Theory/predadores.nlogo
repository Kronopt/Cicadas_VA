;; falta:
;; - limitar o número de filhos limite
;; - impedir a predação de cicadas hidden
;; -

breed [cicadas cicada]
breed [predators predator]

predators-own [lifecycle-duration-ticks energy]
...
to go
  ...
  patches-check-cicadas
  tick
end

to setup-predators
  set-default-shape predators "eagle"
  create-predators ( n-cicadas / 10 ) [                ;; nº de predadores será sempre 1/10 do número de cicadas (convém limitar o slider "n-cicadas" a um mínimo de 10)
    setxy random-xcor random-ycor
    set color brown
    set size 2.5
    set lifecycle-duration-ticks ( ticks-a-year * 3 )  ;; predadores começam com um ciclo de vida de três ano (arbitrário)
    set energy 2
    ]
end
...
to predator-reproduction
  ask predators [
    if (ticks mod lifecycle-duration-ticks) = lifecycle-duration-ticks - 1 [      ;; Os predadores reproduzem-se no início do ano
      if random 100 < 50 [                                                        ;; Produz zero ou duas crias (mais quando come cicadas)
        hatch energy [
          predator-mutation
          set energy 2
          right random-float 360
...
to predator-mutation
  if random-float 100 <= 2 [                           ;; valor de mutação arbitrário (pode ser mudado se for implementado um slider)
    ifelse random 100 < 50
    [set lifecycle-duration-ticks (lifecycle-duration-ticks + ticks-a-year)]
    [if lifecycle-duration-ticks != ticks-a-year [set lifecycle-duration-ticks (lifecycle-duration-ticks - ticks-a-year)]]
    ]
end
...
"" ""
PENS
"default" 1.0 0 -6459832 true "" "plot count predators"
...
PENS
"default" 1.0 1 -16777216 true "" "histogram ([lifecycle-duration-ticks / ticks-a-year ] of predators)"
